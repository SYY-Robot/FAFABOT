	...
	: swSer(0), ss_hw(0), ss_sw(0), m_flag(false), m_rx(rx), m_tx(tx)
{
	...
	{
		m_flag = true;
		ss_hw = new InnerSensor<HardwareSerial>(&Serial4);
		return;
	}
	if (rx == PD2 && tx == PC12)
	{
		m_flag = true;
		ss_hw = new InnerSensor<HardwareSerial>(&Serial5);
		return;
	}
...
#elif defined(__AVR_ATmega32U4__) && !defined(CORE_TEENSY) || defined(__AVR_ATmega644P__) || defined(__AVR_ATmega644__) || defined(__SAMD21G18A__) || defined(__arc__)
